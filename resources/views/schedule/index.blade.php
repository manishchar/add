        ...
        @if($locations)
          @foreach ($locations as $key => $location) 
            <option value="{{ $location->id }}"
              ...
              ?>
            >{{ $location->screen_name.'=>'.$location->location.' = '.$location->id }}</option>
          @endforeach
        @endif
    </select>
  </div>
        ...
        <p id="modalContainer">
          ...
          <div class="row">  
              ...
              <div class="col-sm-6">
                ...
                <div class="col-sm-12">
                  <label>Total Duration Slot : <span id="totalSlotEdit"></span></label>
                </div>
                <div class="col-sm-12">
                  <label>Remaining Slot : <span id="remainingStotLabelEdit"></span>&nbsp;(&nbsp;<span id="remainingSlotSecondEdit"></span>&nbsp;)&nbsp;
                  </label>
                </div>
                <div class="col-sm-12">
                ...
                <input type="hidden"  name="remaining_slot" id="remaining_slotEdit" value="" />
                <input type="hidden"  name="uploaded_slot" id="uploaded_slotEdit" value="" />
                ...
                <div class="col-sm-6">
                  ...
                  <input name="file" id="videoEdit" type="file" class="form-control" onchange="videoChange(this,'Edit')"><br/><div class="progress">
                    ...
                    <input  id="uploadEdit" type="submit"  value="Update" class="btn btn-success" >
                  </div>
...
function edit(id){
  ...
  $.ajax({
    ...
    success: function(result){
      ...
      console.log(obj.advertises.videoTotalLength);
      ...
      var totalSecond = "{{ config('constants.SLOT_TOTAL') }}";
      var totalSlot = "{{ config('constants.SLOT') }}";
      var videoLength = obj.videoLength;
      var videoSlot = Math.ceil(parseFloat(parseInt(videoLength)/30));
      var remainingSlot = (totalSlot-videoSlot);
      var remainingTime = (parseInt(totalSecond)-parseInt(videoLength));
      ...
      // $('#durationLabelEdit').html(remainingTime);
      ...
      $('#totalSlotEdit').html(totalSlot);
      var $source = $("#sourceEdit");
  $source[0].src = "{{ URL('') }}/public/tmp/"+obj.advertises.videoUrl;
  $source.parent()[0].load();
      //$('#sourceEdit').src = ;
//var videoLength = obj.videoLength[i];
var previousVideoSlot = Math.floor(parseFloat(parseInt(obj.advertises.videoTotalLength)/30));
//remainingSlotSecondEdit
      $('#uploaded_slotEdit').val(previousVideoSlot);
      $('#remaining_slotEdit').val(remainingSlot);
      $('#remainingStotLabelEdit').html( remainingSlot);
      $('#remainingSlotSecondEdit').html( remainingTime);
        ...
        success: function(result){
          ...
          if(obj.status == "success"){
            ...
            var sendingData={
                  ...
                  videoFTPPath:obj.data[0].videoUr,
                  ...
                  totalslot:obj.data[0].client.totalslots,
                  avilsolt:obj.data[0].client.avilslots,
                  consumeslot:obj.data[0].client.consumslots,
...
$(document).ready(function() {
    getRecords();
//alert("{{ config('constants.SLOT_TOTAL') }}");
    ...
    //     }
    // }
 // function uploadFormSubmit(id){
 //   return false;
 // }
 // function uploadFormSubmit(id){
 //  alert(id);
 //  // $('#uploadForm'+id).submit(function(){
 //  //   return false;
 //  // });
 //  uploadForms(id);
 // }
      ...
      $.ajax({
        ...
        success: function(result){
          ...
          var obj = JSON.parse(result);
            ...
            var duration = $("#duration"+id).val();
            var iteration = $("#iteration"+id).val();
            var remainingStotSecond = $('#remainingSlotSecond'+id).html();
            console.log('resss',parseInt(remainingStotSecond)-(parseInt(duration)*parseInt(iteration)));
            $('#remainingSlotSecond'+id).html(parseInt(remainingStotSecond)-(parseInt(duration)*parseInt(iteration)) );
            var remainingSlot = $("#remainingStotLabel"+id).html();
            var uploaded_slot = $("#uploaded_slot"+id).val();
             $("#remainingStotLabel"+id).html((parseInt(remainingSlot) - parseInt(uploaded_slot)));
            //$("#remainingStotSecond"+id).html('( '+vid.duration+' )');
            $('#example').DataTable().ajax.reload();
            // $('#remainingDuration'+id).html();
             //console.log(result);
        },error: function(data){
...
function checkSize(rowId){
  ...
  var uploaded_slot = $("#uploaded_slot"+id).val();
  var remainingSlot = $("#remainingStotLabel"+id).html();
  var remainingSlotSecondEdit = $("#remainingSlotSecond"+id).html();
  var duration = $("#duration"+id).val();
  //(duration*iteration)<
  console.log('iteration = '+iteration);
  console.log(' uploaded_slot= '+uploaded_slot);
  console.log('remainingSlot = '+remainingSlot);
  console.log('remainingSlotSecondEdit = '+remainingSlotSecondEdit);
  console.log('duration = '+duration);
  if( (parseInt(duration)*parseInt(iteration) ) <= parseInt(remainingSlotSecondEdit) ){
    return true;
  }else{
    return false;
  }
...
$(document).on('change','.iteration',function(event) {
  ...
  //alert(id);
  var remainingDuration = $("#remaining_slot"+id).val();
  var remainingSlotSecondEdit = $("#remainingSlotSecond"+id).html();
  var remeningSecond = (parseInt(remainingDuration)*30);
  if(checkSize( id )){
    //alert('upload');
    $('#upload'+id).attr("disabled", false);
  }else{
    alert("Remaining second: "+remainingSlotSecondEdit+", Your file too large");
    
     $('#upload'+id).attr("disabled", true);
     //$('#iteration'+id).attr("disabled", true);
        return false;
  }
});
function videoChange(obj,id){
    ...
    vid.onloadeddata = function() {
      //uploaded_slot
      var videoDuration = parseInt(vid.duration);
      var videoSlot = Math.floor(parseFloat(videoDuration/30));
      console.log('mmmmm'+videoSlot);
      var newRemainingSlot = (parseInt(remainingDuration)-parseInt(videoSlot));
      $("#uploaded_slot"+id).val(videoSlot);
      
      $("#duration"+id).val(videoDuration);
      $("#durationLabel"+id).html(parseInt(vid.duration));
      //$("#remainingStotSecond"+id).html('( '+vid.duration+' )');
      if(checkSize(id)){
      console.log('upload success');
      //$("#remainingStotLabel"+id).html(newRemainingSlot);
      $('#upload'+id).attr("disabled", false);
      $('#iteration'+id).attr("disabled", false);
      }else{
      alert("Remaining second: "+remainingSlotSecondEdit+", Your file too large");
      $(obj).val('');
      $('#upload'+id).attr("disabled", true);
      ...
      success: function(result){
       ...
       // console.log(obj.advertises);
          if(obj.advertises.length>0){
           ...
           // upload +="<div>totalSlot"+totalSlot+"</div>";
            for(var i=0;i<obj.advertises.length;i++){
              var obj1 = obj.advertises[i];
              var videoLength = obj.videoLength[i];
              var videoSlot = Math.ceil(parseFloat(obj.videoLength[i]/30));
              var remainingSlot = (totalSlot-videoSlot);
              var remainingTime = (parseInt(totalSecond)-parseInt(videoLength));
              upload +='<div class="col-md-4">';
              upload +='<div>'+obj1.location.screen_name +'</div>';
              upload +='<video id="player'+i+'"  width="100%" controls><source id="source'+i+'" src="{{ URL("public/tmp/1541831639.mp4") }}" type="video/mp4">Your browser does not support HTML5 video.</video>';
              upload +='<div><label>Total Slot : '+totalSlot+'</label></div>';
              upload +='<div><label>Remaining Slot : <span id="remainingStotLabel'+i+'">'+remainingSlot+'</span>&nbsp;(&nbsp;<span id="remainingSlotSecond'+i+'">'+remainingTime+'</span>&nbsp;)&nbsp; Seconds</label></div>';
              upload +='<div><label>Duration in Second -</label><label id="durationLabel'+i+'"></label></div>';
              upload +='<form id="uploadForm'+i+'" fid="'+i+'" enctype="multipart/form-data" onsubmit="return testUpload(this)">';
              upload +='<input type="hidden"  name="_token" id="csrf-token" value="{{ Session::token() }}" />';
              upload +='<input type="hidden"  name="remaining_slot" id="remaining_slot'+i+'" value="'+remainingSlot+'" />';
              upload +='<input type="hidden"  name="uploaded_slot" id="uploaded_slot'+i+'" value="" />';
              upload +='<input type="hidden"  name="advertise_id" id="advertise_id" value="'+obj1.id+'" />';
              upload +='<input type="hidden"  name="duration" id="duration'+i+'" value="" />';
              upload +='<div class="form-group">';
              upload +='Itration<input disabled type="number" placeholder="Itration" name="iteration" rid="'+i+'" id="iteration'+i+'" class="form-control iteration" value="1" min="1">';
              upload +='At End<select class="form-control" name="at_end"><option value="1">Delete</option><option value="2">Remaning</option></select>';
              upload +='<input name="file" id="video'+i+'" type="file" class="form-control" onchange="videoChange(this,'+i+')" required><br/>';
              upload +='<div class="progress">';
              upload +='<div id="bar'+i+'" class="bar"></div >';
              upload +='<div class="percent">0%</div >';
              upload +='</div>';
              upload +='<div id="percent'+i+'">&nbsp;</div><div id="statusMessage'+i+'">&nbsp;</div>';
              upload +='<input  id="upload'+i+'" type="submit"  value="Upload" class="btn btn-success" >';
              upload +='</div>';
              upload +='</form>';
              upload +='</div>';
