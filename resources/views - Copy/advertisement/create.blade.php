    ...
    <div class="col-lg-12">
      <div class="card m-b-30">
        <div class="card-body"> @if(Request::segment(4)==='edit')
          {{ Form::model($location, array('route' => array('advertise.update', $location->id), 'method' => 'PUT')) }}
          <?php  
                $name = $location->name;
            ?>
          @else
             ...
             <div class="row">
              ...
              <div class="col-md-12">
                <div class="form-group"> 
                  ...
                  <div> 
                    <input type="text" placeholder="Enter Advertisement Name" name="advertise_name" id="advertise_name" class="form-control">
                   </div>
                </div>
              </div>
                ...
                <div class="form-group"> 
                  <div> 
                    <label>Location : <span class="text text-danger">*</span></label>
                     <select class="form-control selectpicker" multiple="" name="location_id"> 
                      <option>Select Client</option>
                      @if($locations)
                        @foreach ($locations as $key => $location) 
                          <option value="{{ $client->id }}">{{ $location->location }}</option>
                        @endforeach
                      @endif
                    </select>
                   </div>
